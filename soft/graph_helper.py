...
# <GetMyOrganizationDrives>
def get_my_organization_drives( token, timezone ):
  ...
  # Send GET to /sites/root
  drives = requests.get('{0}/sites/root'.format(graph_url),
    headers=headers)

  return drives.json()
# </GetMyOrganizationDrives>

# <GetIDfromOneDriveOfASite>
def get_drives_ids( token, id_sitio, timezone ):
  ...
  # Send GET to /sites/[ID]/drives
  drives = requests.get('{0}/sites/{1}/drives'.format(graph_url, id_sitio),
    headers=headers)

  return drives.json()
# </GetIDfromOneDriveOfASite>

# <GetListfromOneDriveID>
def get_list_drive( token, id_drive, timezone ):
  ...
  # Send GET to /drives/[ID]/root/children
  drives = requests.get('{0}/drives/{1}/root/children'.format(graph_url, id_drive),
    headers=headers)

  return drives.json()
# </GetListfromOneDriveID>

# <GetListfromFolderFromOneDriveID>
def get_list_drive_folder( token, id_drive, id_folder, timezone ):
  ...
  # Send GET to /drives/[ID]/items/children
  drives = requests.get('{0}/drives/{1}/items/{2}/children/'.format(graph_url, id_drive, id_folder),
    headers=headers)
